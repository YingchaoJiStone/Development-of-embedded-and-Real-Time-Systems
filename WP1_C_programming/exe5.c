...
int main (int argc, char *argv[]){ // Add arguments to instead MAX and MAXNUMBER
    ...
    if(argc == 3){ // If there are 3 arguments
        MAX = atoi(argv[1]); // Set the MAX's value to argument
        MAXNUMBER = atoi(argv[2]); // Set the MAXNUMBER's value to

    }else if (argc == 1) { // If there are no argument
        // Use the default value of MAX and MAXNUMBER
    }else{
